...
test.describe('Pruebas de la API de Comprobante Pago con Excel', () => {
  ...
  // 1. Hook para exportar resultados al finalizar el archivo de tests
  test.afterAll(async ({ }, _testInfo) => {
    // Solo exportamos si hay resultados capturados
    if (resultadosValidacion.length === 0) {
      console.log('⚠️ No hay resultados para exportar. Posiblemente el test no se ejecutó o falló inmediatamente.')
      return
    }

    // Generar el resumen y exportar a Excel
  ...
  test('Enviar peticiones con valores no válidos y validar respuestas correctas', async () => {
    ...
    for (let i = 0; i <= datos.length; i++) {
      ...
      const comprobanteDigitalSignatureAttachmentExternalReferenceURI = procesarValorCeldaExcel(
        fila['comprobanteDigitalSignatureAttachmentExternalReferenceURI']
      )
      ...
      const _comprobanteInvoiceLineUnitCodigo = procesarValorCeldaExcel(fila['comprobanteInvoiceLineUnitCodigo'])
      ...
      const _comprobanteOrdenCompra = procesarValorCeldaExcel(fila['comprobanteOrdenCompra'])
      ...
      const _importeDetraccion = procesarValorCeldaExcel(fila['importeDetraccion'])
      const _montoNetoPago = procesarValorCeldaExcel(fila['montoNetoPago'])
      ...
      const response = await comprobantePagoRest.postComprobantePago(body) // Ejecutas el request
      const tiempoFinManifest = performance.now()
      const tiempoRespuestaRequestMs = tiempoFinManifest - tiempoInicioManifest
      const tiempoRespuestaRequest = tiempoRespuestaRequestMs / 1000 // Convertir a segundos
      ...
      if (statusObtenido === statusEsperado) {
        ...
        if (normalizeForComparison(JSON.stringify(bodyResponse)) === normalizeForComparison(bodyResponseEsperado)) {
        ...
        // Si el status no es el esperado, asumimos que hay un error
      ...
      resultadosValidacion.push({
        ...
        tiempoRespuestaRequest: tiempoRespuestaRequest
      })

      console.log(
        `✅ Fila procesada: ID testcase ${idTestCase} - Status Correcto?: ${statusCorrecto} - Body Response Correcto?: ${bodyResponseEsperadoCorrecto} - Tiempo Manifest: ${tiempoRespuestaRequest.toFixed(3)}s`
      )
    }
  })
})
